...
function main {
    ...
    if [ ! -d ${BITBUCKET_INSTALL_DIR}/.install4j ] || [ ! -f ${BITBUCKET_HOME}/.version ]; then
        ...
        cp -pr /usr/share/atlassian/bitbucket/driver/mysql-connector-java-5.1.45-bin.jar ${BITBUCKET_INSTALL_DIR}/app/WEB-INF/lib

        # Change ownership of the Java Mysql connector
        chown ${BITBUCKET_USER}:${BITBUCKET_GROUP} ${BITBUCKET_INSTALL_DIR}/app/WEB-INF/lib/mysql-connector-java-5.1.45-bin.jar
